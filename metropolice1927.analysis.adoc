:hardbreaks:
die regie = direction

= sinnspruch: MITTLER ZWISCHEN HIRN UND HÄNDEN MUSS DAS HERZ SEIN !!!!!!!!!!

der Mittler = the middler, intermediary

here it means the mediator.

zwischen : btween

das Hirn = brain (here i think it can also mean head)
die Hand = hand 
die+ Hände



muss ... sein : must be ...

and
das Herz    = heart
we already knew that.

so it means:


____
ahem...
____

=== :**THE MEDIATOR BTWEEN THE BRAIN AND THE HANDS MUST BE THE HEART !!!!!!!**


____
echo ...
____


= schicht!
die schicht = shift


== Tief unter der Erde lag die Stadt der Arbeiter

tief : deep
unter: under 
die Erde = earth
but its dative so we use der
lag : past tense of liegen : to lay
die+ Arbeiter = workers
and der Arbeiter becomes of the workers

so it becomes:

=== :Deep beneath the earth lay the city of the workers


== So die Stadt der Arbeiter unter der Erde lag, so hoch über ihr \
türmte sich der Häuserblock, der "klub der Söhne" hiess, mitt seinen Hörsälen \
und Bibliotheken, seinen theatern und Stadions.


so ... so ...:
just as ..., so ...

über : above
ihr : here it is refering to the "die Stadt" not ihr = you plural
der Turm : tower
türmten : to tower
türmte : it towered


der Häuserblock = the residental block
hiess is old german for hieß
die+ Hörsäle        = lecture rooms
die+ Bibliotheken   = librarires
die+ theatern       = theaters 
die+ stadions       = stadiums


:mean1: as the city of the worksers lay deep under ground, so did tower haigh \
the residental above it, called the club of the sons. with its lecture rooms \
and librarries and theaters and studiums.
=== :{mean1}


:head2: Wie kommt es josaphte, dass ich durch meinnen sohn von der Explosion \
erfahre, statt von Ihnen !
== {head2}

wie kommt es dass: littarly: how come it, meaning:
"how is it that?"

erfahren : to find out / learn / hear about
durch   : through/via
meinnen : again its the acusative of the mein

statt   : insted of 
von     : from
Ihnen   : you in dative



=== :how the fuck am i hearing this from my son not you dumass?


die+ Einzelheiten = elaboration

== Was hattest du in den Maschinensälen zu suchen, Freder?
was : what
hattest : had 

|===
|die+ Maschinensälen	| the machine halls

|die+ Machinen   		| machines 
|die+ sälen      		| halls
|===

suchen          : to search
zu suchen haben : to have a business 

litteral tanslation:
what did you have to look for in the machine halls?

natural translation:

=== :the fuck were you doing in the machine halls?


:head3: Ich wollte den Menschen in die Gesichter sehen,deren kline kinder \ 
meine Brüder, meine Schwestern sind.
== {head3}


Ich wollte : means i wanted, 
what ever comes after it is what "Ich" wanted

die+ Menschen   = people/ plural of person 
die+ Gesichter  = faces

deren : means whose.

kline --adj--> small != groß
and 
die+ kinder = children.
we already knew that

and 
die+ Brüder = brothers
die+ Schwestern = sisters 

sind is conjegated here and is being controlled by the deren at the beginning 


:mean4: i wanted to look into the faces of the people, whose small children \
are my brothers and sisters.
=== :{mean4}


____
can I just write down that i love this scene?. I really love this scene.
____

:head4: Deine herrliche Stadt, Vater und Du das Hirn dieser Stadt und wir alle \
im Licht dieser Stadt... 
== {head4}

herrliche --adj--> shiny, glrious

dieser : this, such

das Licht = light 

:mean2: your glrious city, father and you the brain of it, and all of us in \
the light of this city...
=== :{mean2}


== und wo sind die Menschen, Vater deren Hände Deine Stadt erbauten ?

bauten : to build
erbauten: gives bauten a more formal and literary meaning, 
something like : bring to existance.

=== :and where are the people, father whose hands built your city?


== Wo sie hingehören

|===
|gehören 	| to belong 

|+ 			| 
|hin 		| to belong (toward somewhere spicific)
|===

=== :where they blong


== ... wo sie hingehören ... ?

____
already translated
____


== in die Tiefe ?
die Tife = depth


== Und wenn die in der Teife einmal aufstehen gegen Dich?


wenn  : when 

die in : those in
einmal : once, her it means 'one day' or 'someday'


|===
|stehen | to stand 

|+ 		|
|auf	| to stand up, to rise up
|===


gegen : against, in oposition to

=== :and when those in the depth one day rise up against you?


== der Erste Werkmeister an der Herz-Maschine Grot, mit wichtiger Meldung

erste = first 
erste --adj--> head, lead


|===
|der werkmeister 	| workmaster

|das werk 			| factory 
|der meister 		| master
|===


Grot : here its the name 

wichtiger --adj--> important == Groß
die Meldung = message

=== :head workmaster at the heart machine Grot, with important messege


== Da sind wieder zwei von den verdammten Plänen, Herr Fredersen


Da sind : there are
wieder zwei : two more (again two)

verdammten --adj--> fucking 
die+ Pläne = plans, maps


=== :there are two more of those fucking maps mr.Fredersen


== in den Taschen von zweien, die heut an der M-Machine verunglückt sind ...

das Tasche = pouch 

zwei = obviosly 2 
zweien = two thing, 2 people,


verunglückt : with accident

=== :in the pockets of 2 of the people who had an accident in the machine today.


:head5: Wie kommet es, josaphte, dass mir diese Pläne von Grot gebracht werden \
, statt von Ihnen?
== {head5}

dass mir ... gebracht werden : being brought to me

:mean5: how the fuck am i recieving these maps from Grot and not from your \
useless ass?
=== :{mean5}


== Die G-Bank wied Ihnen Ihr Restgehalt auszahlen

das Gehalt = salary
das Restgehalt = remanning salary

|===
| zalen | to pay

|+ 		|
|aus	| to pay out, to get even
|===


werden + infinitive is how a future sentence is constructed so
'wied ... auszhlen' means: will pay out

=== :the G-bank will pay you your remanning salary (get the fuck out!)


____
and bro doesnt even try to defend himself!
DAMN!
____

:haed6: Vater weisst Du, was das heisst, so von Dir entlassen zu werden? \
Das heist: hinunter! - Vater hinunter! In die Tiefe!
== {haed6}

weisst is weißt in moder german and conjugated wissen meanning to know
was das heisst : what that means... 
(here heisst meanning called can also mean to mean something) 

entlassen : to dissmiss, to fired

so von Dir entlassen zu werden : to be fired by you

hinunter: downward, go down


:mean6: father you know what it means to be fired by you! it means: go down! \
go down to the depth
===  :{mean6}

== Wissen Sie, was das heisst, von Joh Fredersen so entlassen zu werden?

here Sie is obvoiusly talking in a formal and respectfull way 
(Freder is some sort of a prince afterall).

=== :do you know what it means to be fired by Joh Fredersen?


== Wollen sie zu mir kommen Josaphet?

=== :do you want to come to me josaphte?


:head7: Gehen Sie heim, Josaphate, und warten Sie auf mich... \
Ich habe heute nacht noch einen weiten Weg zu gehen...
== {head7}

das heim = home

warten : to wait, to hold


here 
haben + noun + zu + infinitive
is interesting
it means having something to do

einen weiten Weg : a long path

=== :go home josaphate and wait for me. i have a long way to go tonight.


== in die Teife zu mienen Brüdern...
=== :in the depth to my brothers...


:head8: von heute an wünsche ich, über jeden Schritt meines Sohnes genau \
unterrichtet zu werden ...
== {head8}

von heute an : from today onward

wünschen : to wish
der Schritt = step

genau --adj--> exactly

unterrichten : to informe


=== :starting from today, i want to be informed of every exact step my son takes


== Bruder

=== :bro!


== die Maschine! ... Es muss ein Mensch an der Machine sein!

der Mensch = person, human

=== :the Machine! someone must be at the Machine!


== Es w i r d ein Mensche an der Maschine sein

Es wird ... sein : there will be ...

=== :someone will be at the Machine!


== ICH

=== :ME !!!!


== Höre mich an... ich will mein Leben mit Dir tauschen...

Höre mich an: hear me out

tauschen : to swap

=== :hear me out, i want to swap places with you


== Wartet auf mich beide
Wartet : to wait
beide : both

=== :wait for me (both of you)


:head9: Mitten in Metropolis lag ein seltsames Haus, das die Iahrhundrte \
vergessen hatten
== {head9}


Mitten --adj--> middle
seltsames --adj--> strange

das Jahrhundert = century
die+ Jahrhunderte = centuries

vergessen : to forget

:mean7: in the middle of metropolis lay a strange house that the centuries \
had forgotten
=== :{mean7}


== Der Mann, der darin whohnt war Rotwang, der Erfinder

=== :the man who lived there was Rotwang the inventor


____
he's got that starwars-ass hand too
____


== Joh Fredersen...


== Ein Hirn wie das Deine, Rotwang, müsste vergessen können...

=== :a brain like yours, Rotwang, must be able to forget


____
oscar level acting befor the oscars were even a thing!
____

:head10: Ich habe ein einziges Mal im Leben etwas vergessen: Dass Hel ein Weib \
war und Du ein mann
== {head10}

:mean8: i have forgotten somethong in my life only once: that hel was a woman \
and you were a man
=== :{mean8}


== lass die Tote ruhen, Rotwang... Sie ist Dir wie mir gestorben...

ruhen : to rest
gestorben --adj--> dead

=== :let the dead rest Rotwang... she is as dead to you as she is to me


== Mir ist sie nicht gestorben Joh Fredersen, mir lebt sie !!!
=== :she is not dead to me Joh Fredersen. to me SHE LIVES!!!!


:head11: Glaubst Du, der Verlust einer Hand sei zu hoher Preis für \
die Wieder-Erschaffung der Hel?
== {head11}

Glaubst : to think 
der Verlust = loss

die WiederErschaffung = recreation

:mean9: do you think the loss of a hand is too high of a price for the \
recreation of Hel?
=== :{mean9}


== Willst du sie sehen?

=== :wanna see her?


____
HOW THE FUCK DID THEY MAKE THIS?
____

:head12: Nun, Joh Fredersen ?! Lohnt es sich nicht, eine Hand zu verliren, um \
den Menschen der Zukunft den M a s c h i n e n - M e n s c h e n geschaffen zu \
habben ?! 
== {head12}

:mean10: Well, Joh Fredersen? Isn't it worth losing a hand? in order to have \
created the Machine-Man for the people of the future?!
=== :{mean10}


:head13: Noch 24 Stunden Arbeit und kein Mensch, Joh Fredersen, wird den \
Maschinen - Menschen von eiem Erdgeborenen unterscheiden können !
== {head13}

der Erdgebornen : old german
|===
|der Erdgeborenen 	| earthborn, born on earth

|die Erde 		| earth
|der geboren 	| some one who has been born
|===

unterscheiden : to distinguish

:mean11: in 24 hours of work, no one can distinguish the machine man form an \
ordenery earth-born
=== :{mean11}


== Mein ist die Freau, Joh Fredersen! Dir bleib der Sohen der Hel!

bleib : stay

=== :mine is the woman, with you stays the son of hel!


____
mainiac!
____

== Und was führt Dich zu mir Joh Fredersen?

führen : to lead, to guid
=== :and brings you here to me Joh Fredersen?


== Ich brauche Deinen Rat wie stets, wenn alle meine Sachverständingen versagen.

der Rat = advice
die+ Sachverständingen = experts 
versagen : to fail

=== :as always, i need your advice when all my experts fail


:head20: Kannst Du mir dise Pläne enträtseln, die man seit Monaten in den \
Taschen meiner Arbeiter findet ? 
== {head20}

man: people, they

:mean12: can you decifer these maps for me? they are being found in the \
pockets of workers for months
=== :{mean12}


== ...um zwei nach dem Schichtwechsel! Sei hat weider gerufen...

|===
|der Schichtwechsel	| shift switch

|der wechsel 		| switch
|===

gerufen : to summon

=== :at 2 after the shift witch! she has summoned again


== Vater - Vater ! Nehmen zehn Stunden niemals ein Ende ??!!

Nhemen ... ein end: take an end
fixed expressin , like "coming to an end""
niemals : ever, never

=== :father father , does this ten hour ever end?


:head14: es ist ein plan der zweitausendjährigen katakomben tief under den \
Tiefbahnen Deiner Metropolis
== {head14}

:mean13: it is a map of the 2000 years old catecombs running deep under the \
underground railways of your metropolis
=== :{mean13}


:head15: Ich möcht doch herausbekommen, was meine Arbeiter in den Katakomben \ 
zu tun haben
== {head15}

heraus : out 
bekommen : to recive
herausbekommen : to find out

=== :i want to find out what my workers have to do in the catecombs


= STORY TIME !!
== heute will ich euch die Legende OM TURMBAU ZU BABEL erzählen...

erzählen : to narrate

=== :today i want to tel you the story of the tower of babel


== Auf! Lasset uns einen Trum bauen, dessen Spitze bis an die Sterne reiche!
Auf : Up

here Auf means come on, its inviting the audiance to something.

Lasset uns .... bauen: lets build ...
____
lest we build a FUCKING TOWER !!!!
____

----
from the article analysis session: 
einen tells us the thing being build is TOWER
----

dessen : whose, of which

die+ Sterne = stars

=== :COME! LETS US BUILD A TWOER WHOS TOP REACHES THE STARS


:head16: Und an die Speitze des Turmes wollen wir schriben: Gross is die Welt \
und ihr Schöpfer! Und gross ist der Mensch!
== {head16}

die Welt = world
der Schöpfer = creator, originator

:mean3: and on top of it we shall write: \
"GREATE IS THE WORLD AND ITS CREATOR AND GREAT IS THE MAN \
(or parhaps the mankind)"
=== :{mean3}


:head17: Aber die den Trum Babel erdachten, konnten den Trum Babel nicht bauen \
Allzu gross war das Werk Da warben sie fremde Hände um Lohn...
== {head17}

erdenken : to devise, to come up with
allzu : all to, way too, overly

der Lohn = wage


:mean14: but those who came up with the tower couldnt build the tower, \
it was to great of a work, so they hired foriegn hands on wages
=== :{mean14}


:head18: Aber die Hände , die den Trum Babel erbuten, wussten nichts von dem \
Traum, den das HIRN das ihn erdacht hatte träumte.
== {head18}

der Traum = dream

:mean15: but the hands, those who build he tower knew not of the dream, that \
the brains had dreamed
=== {mean15}


____
i love how the 1920's view of the upper class is : 
"the smart and eduacated people that are some times crule"
wheras now, we dont even see them as smart, thair just the assholes with money
the avrage corperate worker is many times more educated then a billioner 
____

____
greate vissuals BTW
____

== Lobgesang des Einen wurde Fluch der Andern.
der Lobgesang = songsof prais
Andern : old german of Anderen
die+ Andere = others

der Fluch = curse 

=== :songs of prais became curses of others.


== Gleiche Sprache sprechend. verstanden die Menschen sich nicht...

=== :tho speaking the same language, the people did not understand each other.


== Einen Mittler brauchen HITN und HÄNDE

=== :a mediator btween the brain and the hands is needed


== sinnspruch: MITTLER ZWISCHEN HIRN UND HÄNDEN MUSS DAS HERZ SEIN !
____
yeah we got the messege the first time lady
____

= story time is over :D


== UND wo ist unser Mittler, Maria ?
unser : our

=== :and who is our mediator maria ?


== Wartet auf ihn! Er kommt gewiss !
Wartet auf, aufwartet : to wait (for someone)

=== :have patiance! he will come for sure !


== Wir werden warten Maria...! Aber nicht lange mehr !

=== :we will wait maria! but not for long


== Mittler Du, bist Du endlich gekommen...

=== :oh mediator, have you finaly came?


== Du hast mich gerufen da bin ich

=== :you have summond me, here i am !


== Rotwang gib dem Maschinen-Menscen das Gesicht dieses Weibes

=== :Rotwang givethe machine man, this woman's face


____
evil ass CEO right here
____

:head19: Ich wil Zwietracht säen zwischen ihnen und ihr! \
Ich will ihren Glauben an diese Frau zerstören
== {head19}

zerstören : to destroy

=== :i want chaos btwen them, i want thair faith in this woman destroied 


:head21: Lass mich nun allein, Joh Fredersen... Du wirst den Rückweg ohne mich \
finden...
== {head21}

nun : now

=== :new leave me alone Joh Fredersen, you can find your way back without me ...


:head22: Du Narr! nun sollst Du auch das Letzte verlieren, das Du von Hel \
noch hattest... D e i n e n  S o h n ...
== {head22}

der Narr = fool
das Letzte = the last

:mean16: you fool! now you shall lose the last thing you have left from Hel, \
your son!
=== :{mean16}


== Auf morgen, im Dom!

=== :in the morning, in the church!


== Ende des Auftakts

=== :end of the prolog

____
PROLOG ?!
bro that was an entire hour !
____


= **PART 2 I GUESS**

== Zweischenspiel

|===
| das Zwischenspiel | middle act 

| zwischen 			| between
| das speiel 		| play
|===


:head23: Auf der kanzel erblickt Freder einen Mönch, der predigt: "Wahrlich, \
ich sage Euch: Nahe sind die Tage, von denen die Apokalypse spricht!"
== {head23}

der keanzel = pulpit (its the raised platform of the curch where a prist stands)
erblicken 	: to behold
predigen 	: to preach

Wahrlich : truly, verly
Nahe --adj--> near

:mean17: on the pulpit Freder sees a monk, he priches: "verly, i tell you: \
near are the days, of which the apocolypse speaks of!"
=== :{mean17}


== Die Hand des Mönchs weist auf die Bibel, in der geschreiben steht:

weisen : to point

=== :the monk's hand points to the Bibel, where it was written:


____
wow thats a alot of info 
im just gonna translate the whore of babylon part
____

== ... Die grosse Babylon

=== :the grate babylon


== Du wirst Joh Fredersen vernichten - ihn und sein Stadt und seinen Sohn !

=== :you will destroy Joh Fredersen, him and his city and his son !



____
look the 7 deadly sins are here
____

== **written on the statues**
|===
| VÖLLEREI	| GLOTTONY
| GEIZ		| GREED
| EITELKEIT	| PRIDE
| WOLLUST	| LUST
| NEID		| ENVY
| ZORN		| WRATH
| TÄGHEIT	| SLOTH
|===






