= verbs
== common verb form

|===
|ich       	| bin   | brauche
|du         | bist  | brauchst
|er/sie/es  | ist   | braucht
|wir        | sind  | brauchen
|ihr        | seid  | braucht
|Sie        | sind  | brauchen
|===


== common verbs
|===
|brauchen	3+| to need/want
|finden		3+| to find

.4+|komme	3+| to come
		   .3+| +	| an 				| to arrive
					| mit 				| to come along
 					| zurück 			| to come back
|haben 		3+| to have

.2+|fangen	3+| to harvest,fish,collect
			  | + 	| an				|to begin,start

|gehen		3+| to go
|treffe		3+| to meet
|besuchen	3+| to attend (on a regular basis)
|spielen	3+| to play

.2+|stehen	3+| to stand
			  | + | auf  				|to stand up, to rise up
|===

== fucked up verbs
|===
2+|					|ich	|du		|er/sie/es	|wir	|ihr	|Sie         

|essen	| to eat	| esse 	| isst 	| isst 		| essen | esst 	| essen
|wissen	| to know 	| weiß 	| weißt | weiß 		|wiessen| wisst	| wissen
|nehmen | to take 	| nehme | nimmst| nimmt 	| nehmen| nehmt | nehmen
|sehen	| to see 	| sehe 	| siehst| sieht 	| sehen | seht 	| sehen
|===

== modals
|===
2+|					| ich	|du			|er/sie/es	|wir	| ihr	|Sie         

|können	| can 		| kann	| kannst	| kann		| können| könnt	| können
|mögen 	| like to	| mag 	| magst		| mag 		| mögen | mögt 	| mögen
|wollt 	| want 		| will 	| willst	| will 		| wollen| wollt | wollen
|dürfen	| to allow	| darf 	| darfst	| darf 		| dürfen| dürft | dürfen
|muss  	| must 		| muss 	| musst 	| muss 		| müssen| müsst | müssen
|===

= just notes
== phrase 

=== Danke and its bros

* Danke sehr!
* Oh danke!
* Herzichen Dnak!
* Vielen Dank!



== common qustions

    Wie heißt das auf Deutsch?
    what is this in german


    Wie viel Uhr ist es?
    (what is the hour)

    Wie spät ist es?
    (how late is it)
    what time is it

== wie oft

* nie : never 0%
* fast nie ~0%
* manchmnal: 50%
* oft : 75%
* fast immer ~100%
* immer : always 100%

== time preposition table !

|===
| word			| meaning		| Example				| Meaning

| am          	| on            | am Montag             | on Monday
| im          	| in            | im Sommer             | in summer
| um          	| at            | um 8 Uhr              | at 8 o'clock
| vor         	| before / ago  | vor dem Essen         | before the meal
| nach        	| after         | nach dem Essen        | after the meal
| seit        	| since / for   | seit Montag           | since Monday
| ab          	| starting at   | ab Montag             | from Monday onward
| bis         	| until / by    | bis Montag            | until Monday
| von ... bis 	| from ... to   | von Montag bis Freitag| Monday to Friday
| für         	| for (duration)| für zwei Tage         | for two days
| in          	| in / from now | in zwei Tagen         | in two days
|===

im : in dem

== adjssss

Groß ist das Gegenteil von klein : Groß ≠ klein

	Groß ≠ klein

	schön ≠ hässlich

	teuer ≠ billig

	schwer ≠ leicht : hard/heavy ≠ easy/light

	interesstant ≠ lang weiling 

	ruhig ≠ laut 

	neu ≠ alt
	
	fern = weit ≠ nah
	(fern has a more abstaract and poetic weight to it, like its far in terms of 
	time and concept too, while weit convays a pysical long distance)

	zwischen : between


= dictionary
== heap of words
[%hardbreaks%]
die Brille              = glasses
der kugelschreiber      = pen
der Bleistift           = pencil
das Buch                = book
die Flasche             = bottle
die Tasche              = purse
der Drucker             = printer
die+ Drucker            = printers
die+ bälle              = balls
die Zeit                = time
das Klaveir             = piano
der Satz                = sentance, statement
die+ Sätze              = plural of der Satz
die Stadt ≠ das Dorf

=== places
[%hardbreaks%]
die Ausstellung         = exhibition,gallery
die kneipe              = bar
die Schule
der Kindergarten
das Krankenhaus
die Kirche
das Café
das Restaurant
das Haus
die Bank
der Bahnhof				= station
die S-Bahn = train
die U-Bahn = train (under ground and in urban city)
das Geschäft 			= shop
das Büro 				= office
die Wohnung 			= appartment
das Kino                = cinema

==== plurals of the same places
die+ Banhöfe
die+ Geschäfte
die+ Banken
die+ S-Bahnen
die+ U-Bahnen
die+ Büros
die+ Wohnungen
die+ Häuser
die+ Schulen
die+ Kindergärten
die+ Kirchen
die+ Krankenhäuser
die+ Cafés
die+ Resturants
die+ Kinos




== compounds
|===
2+|der Arbeitsplats

| die Arbeit	| work
| der Platz 	| place
|===

|===
2+|das Schwimmbad

| Schwimm	| swimming
| das Bad	| bath
|===


|===
2+|das Herzeleid

| das Herz	| heart
| das leid	| pain, hurt, sorrow, woe
|===


= TIME!!!!!!
== weekdays

**die+ Wochentage **

1 der Montag
2 der Dienstag
3 der Mittwoch
4 der Donnerstag
5 der Freitag
6 der Samstag
7 der Sonntag


=== day times
|===
2+|die+ Tageszeiten (die Tageszeit) 

| der Morgan  	| early
| der Mittag  	| middle
| der Abend   	| end
| die Nacht   	| night
|===

das heute = today

das gestern = oposite of today

=== seasons n month
[cols="h,1,1,1"]
|===
|der Frühling	| der März 		| der April   	| der Mai
|der Sommer   	| der Juni		| der Juli    	| der August
|der Harbst   	| der September	| der Oktober	| der November
|der Winter   	| der Dezember	| der Januar  	| der Februar
|===
